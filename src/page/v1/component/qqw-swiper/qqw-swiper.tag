<qqw-swiper>
	<header class="swiper-container qqw-banner qqw-swiper-box">
		...
		<div id="{ opts.selectorid }" data-url="{ url }" class="swiper-wrapper" if={ opts.brand }>
			<a class="swiper-slide swiper-slide-brand qqw-op-bg" each="{ opts.brand }" href="{ url }">
				<div if="{!more}">
				    <div class="picbox" style="border-radius:{radiusValue}">
				           <div class="swiper-slide-brand-img a-lazy-upload "   data-src='{pic}'></div>
				    </div>
					<p class="swiper-brand-label">{ name }</p>
					<span class="swiper-brand-tip">{ price }</span>
				</div>
				<div class="swiper-slide-brand-more" style="border-radius: {radiusValue}"  if="{more}">
